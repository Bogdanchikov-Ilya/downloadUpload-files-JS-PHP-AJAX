...
async function getDownloads() {
  const res = await fetch('https://cw60005.tmweb.ru/downloads/', {
    method: 'GET'
  })
  ...
  result.forEach((item) => {
    ...
    <div class="card">
      <div class="card-body">
        ...
        <a href="https://cw60005.tmweb.ru/getfile?filename=${item.filename}" id="btn" class="btn btn-primary">Скачать</a>
      </div>
    </div>`)
  })
}
...
document.querySelector('form').addEventListener('submit', async function (e) {
  ...
  const res = await fetch('https://cw60005.tmweb.ru/upload', {
    method: 'POST',
    body: formData
  })
  console.log(res)
